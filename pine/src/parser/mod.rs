...
tag_token!(doublearrow_tag, Token::DoubleArrow);
tag_token!(eof_tag, Token::EOF);


// fn doublearrow_tag(tokens: Tokens) -> IResult<Tokens, Tokens> {
//     tag("=>")(input)
//
//     verify(take(2usize), |t: &Tokens| t.tok[0] == Token::DoubleArrow)(tokens)
//
// }
...
fn parse_stmt(input: Tokens) -> IResult<Tokens, Stmt> {
    alt((parse_let_stmt, parse_fun_stmt,parse_return_stmt, parse_expr_stmt))(input)
}
...
fn parse_let_stmt(input: Tokens) -> IResult<Tokens, Stmt> {
    ...
    map(
        ...
        |( ident, _, expr,_)| Stmt::LetStmt(ident, expr),
    )(input)
}
fn parse_fun_stmt(input: Tokens) -> IResult<Tokens, Stmt> {
    ...
    map(
        ...
        |( ident, expr)| Stmt::LetStmt(ident, expr),
    )(input)
}
...
fn parse_fn_expr(input: Tokens) -> IResult<Tokens, Expr> {
    map(
        ...
        |( _, p, _,_, b)| Expr::FnExpr { params: p, body: b },
    )(input)
}
